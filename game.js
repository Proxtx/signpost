...
export class GameState {
  ...
  findPointedSigns(signCoords) {
    ...
    for (let x in this.grid) {
      for (let y in this.grid[x]) {
        if (this.hits([x, y], signCoords)) pointed.push([x, y]);
      }
    }
  ...
  dulplicateGameState() {
    if (this.gameStateIndex < this.gameStates.length - 1)
      this.gameStates.splice(this.gameStateIndex + 1);
    this.gameStates.push(
      JSON.parse(JSON.stringify(this.gameStates[this.gameStateIndex]))
    );
    this.gameStateIndex++;
    this.loadGameState();
  }
  ...
  findAndExecuteConnectMethod(sign1, sign2) {
    ...
    if (!find1 && !find2) this.createSubPath([sign1, sign2]);
      ...
      this.mergeConnections(
        find1.connection,
        this.createSubPath(sign2),
        find1.index,
        0
      );
    else if (!find1 && find2)
      this.mergeConnections(
        this.createSubPath(sign1),
        find2.connection,
        0,
        find2.index
      );
  ...
  findConnection(sign) {
    for (let connection of this.connections) {
      for (let signOfConnectionIndex in connection.signs) {
        ...
        if (sign[0] == signOfConnection[0] && sign[1] == signOfConnection[1])
          return { connection, index: Number(signOfConnectionIndex) };
      }
    }
  ...
  freeSign(sign) {
    ...
    this.splitConnection(
      1,
      this.splitConnection(foundConnection.index, foundConnection.connection)
    );
  }
  ...
  mergeSubPaths(subPath1, subPath2) {
    ...
    if (
      subPath1.signs.length >= subPath2.signs.length ||
      subPath2.index == undefined ||
      subPath2.index == null
    ) {
  ...
  resolveFinalConnections() {
    for (let connection of this.connections) {
      ...
      for (let matchingConnection of this.connections) {
        if (
          matchingConnection.type != "final" ||
          matchingConnection.signs.length < 1
        )
          continue;
  ...
  hits(sign1, sign2) {
    ...
    for (let sign of signs)
      if (sign[0] == sign2[0] && sign[1] == sign2[1]) hits = true;
...
export class InteractionEngine {
  ...
  interactionInit() {
    ...
    this.activeSign = this.evaluateSign(
    ...
    if (this.type == "pointed") navigator.vibrate(50);
    this.display();
  }
  ...
  evaluateSign(x, y) {
    let xCoord = Math.floor(
      (x - this.renderer.padding) /
        (this.renderer.borderSize + this.renderer.cellSize)
    );

    let yCoord = Math.floor(
      (y - this.renderer.borderSize * 0.5 - this.renderer.padding) /
        (this.renderer.borderSize + this.renderer.cellSize)
    );

    if (this.gameState.grid[xCoord]?.[yCoord]) return [xCoord, yCoord];
...
export class Interaction {
  ...
  constructor(element) {
    ...
    this.element.addEventListener("touchstart", (e) => {
      const coords = this.getRelativeCoordinates(
        e.touches[0].clientX,
        e.touches[0].clientY
      );
